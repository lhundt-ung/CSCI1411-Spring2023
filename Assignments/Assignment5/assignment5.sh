...
TITLE="Security Report prepared for CSCI 1411 - Spring 2023"
...
# 3. Function: target_os
# Purpose: To write out the target's Operating System in HTML
# Use nmap to identify the Operating System of the target system. You may consider 
# using the pipe operator to grep the nmap results for the OS information line only. 
# Consider using the variable that stores the target ip from earlier. 
# This command now requires *Admin* elevation to run, add sudo before the command.
# EXAMPLE: sudo nmap <parameters>
function target_os()
{
...
# Use nmap with a parameter to scan ports with Version detection. 
